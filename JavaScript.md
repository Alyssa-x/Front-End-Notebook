...
 /*实现原理参考自适应正方形和水平居中方式*/.box {  position: absolute;  top: 0;  right: 0;  left: 0;  bottom: 0;  margin: auto;​  width: 10%;  height: 0;  padding-top: 20%;  background: tomato;}css
...
// （4）判断函数的返回值类型，如果是值类型，返回创建的对象。如果是引用/函数类型，就返回这个引用类型的对象/函数。
...
（1）将 document.domain 设置为主域名，来实现不同子域名的跨域操作，这个时候主域名下的 cookie 就能够被子域名所访问。同时如果文档中含有主域名相同，子域名不同的 iframe 的话，我们也可以对这个 iframe 进行操作。
